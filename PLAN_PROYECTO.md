...
| **El orquestador no habla MCP con FreeCAD: lanza `freecadcmd` como subproceso** | F0.4 (`scripts/start-host-mcps.ps1`, que no existe) y F0.5 (`orchestrator/mcp/client.py`, que tampoco). Un MCP de FreeCAD ya levantado en el host **no lo usa nadie**: lo que hace falta es el binario accesible y `FREECADCMD` apuntándolo | **F0.5**, si alguna vez hace falta separar FreeCAD del orquestador | Sin fecha: hoy no estorba |
| **Todo corre nativo en macOS, no en contenedores** | La fase 0 entera: F0.2 (`docker-compose.yml`), F0.4 (puentes MCP), F0.5 (cliente MCP desde el contenedor), F0.7 (traducción de rutas contenedor ↔ host) y § 8.4 de la arquitectura. El supuesto S1 es Windows 11 con Docker Desktop; se ha desarrollado en un portátil Mac con `.venv`, FreeCAD del sistema, OpenClaw por npm global y un LaunchAgent | **F0.2** y **F0.7** | Al pasar a la PC de la 5090 |
...
> Sobre la primera: no es solo empaquetado. Mientras el sistema solo exista como instalación hecha a mano en un ordenador, **mudarlo o reproducirlo es trabajo manual y silencioso** —rutas absolutas en el registro del MCP, un LaunchAgent que solo existe en macOS, secretos que se copian a mano— y nada de eso está verificado por un test. Es la deuda que más se parece a convertirse en el estado permanente, porque no duele hasta el día que cambias de máquina.
